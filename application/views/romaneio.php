...
if ($vendas) {
	...
	for ($i = 1; $i <= count($vendas); $i++) {
		...
		if (file_exists($cliente -> config -> item('base_url') . 'css/img/img_produto/' . $vendas[$i - 1]['foto_produto'])) 
			$pdf -> Image($cliente -> config -> item('base_url') . 'css/img/img_produto/' . $vendas[$i - 1]['foto_produto'], 2 + ($largura * 5.6), 4.5 + ($altura * 5.4), 5.5);
		else
			$pdf -> Image($cliente -> config -> item('base_url') . 'css/img/img_sistema/cinza.jpg', 2 + ($largura * 5.6), 4.5 + ($altura * 5.4), 5.5);
